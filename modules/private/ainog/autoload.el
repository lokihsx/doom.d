...
(defun ainog-graphql/graphqls-file-rearrangement ()
  (interactive)
  (setq type-str "")
  (setq condition-str "")
  (setq input-str "")
  ...
  (with-current-buffer (buffer-name)
    ...
    (while (search-forward-regexp "#.+\ntype \\([A-Za-z]+\\) {" nil t)
      (search-backward "#")
      (setq origin-pos (point))
      (search-forward "}")
      (setq end-pos (point))
      (setq type-str (format "%s\n%s" type-str (buffer-substring origin-pos end-pos))))

    (goto-char (point-min))
    (while (search-forward-regexp "#.+\ninput \\([A-Za-z]+Condition\\) {" nil t)
      (search-backward "#")
      (setq origin-pos (point))
      (search-forward "}")
      (setq end-pos (point))
      (setq condition-str (format "%s\n%s" condition-str (buffer-substring origin-pos end-pos))))

    (goto-char (point-min))
    (while (search-forward-regexp "input \\([A-Za-z]+Input\\) {" nil t)
      (setq origin-pos (line-beginning-position))
      (search-forward "}")
      (setq end-pos (point))
      (setq input-str (format "%s\n%s" input-str (buffer-substring origin-pos end-pos))))

    (goto-char (point-min))
    (while (search-forward-regexp "#.+\nextend type Query {" nil t)
      (search-backward "#")
      (setq origin-pos (point))
      (search-forward "}")
      (setq end-pos (point))
      (setq query-str (format "%s\n%s" query-str (buffer-substring origin-pos end-pos))))

    (goto-char (point-min))
    (while (search-forward-regexp "#.+\nextend type Mutation {" nil t)
      (search-backward "#")
      (setq origin-pos (point))
      (search-forward "}")
      (setq end-pos (point))
      (setq mutation-str (format "%s\n%s" mutation-str (buffer-substring origin-pos end-pos))))
